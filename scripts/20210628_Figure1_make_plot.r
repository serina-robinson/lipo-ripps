...
mat_bool <- mat # makes a copy
mat_bool[mat > 1] <- 1 # dreates a boolean matrix
...
# Remove columns with near zero variance using nearZeroVar fucntion
...
# Normalize across rows using sweep funciton
findat[1:10, 1:10]
...
pal2 <- c(pal[c(2, 8, 5, 1, 4)], "seagreen2", "gray80",  "black")
...
prod <- ggplot() + 
  ...
  geom_text_repel(data = subset(plotdf, !known_product %in% c("unknown", "npu", "ksp", "pha")),
    size = 5,
    aes(x = PC1,
        y = PC2,
        label = known_product)) +
...
colorblind <- c(pal[1], "#CC6677",  "#999933",  "steelblue4", "#AA4499",  
                ...
                "navyblue", "mistyrose1", "#88CCEE", pal_distinct[4], 
...
pdf("output/Figure1_part2.pdf", width = 15, height = 5)
wrap_plots(  
  E = nif11,
  F = abc,
  G = c39,
